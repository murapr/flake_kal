   ...
   systemd.services.franceshadowsocks = {
    ...
    script = ''
     ...
     ss-local \
        ...
        -m "xchacha20-ietf-poly1305" \
        --plugin "v2ray-plugin" \
        --plugin-opts "tls;host=canada.nyaaa.net.eu.org;loglevel=debug" \ 
        -t 300 \
        --reuse-port \
        --fast-open
    '';
...
# services.printing.enable = true;
  ...
  users.users.mur = {
    ...
    packages = with pkgs; [
      ...
      [
        "Exec=bash -c \"cat /run/agenix/asr | keepassxc --pw-stdin /home/mur/Dropbox/Sync/Passwords.kdbx\""
      ])
      rnote
      lutris
      distrobox
